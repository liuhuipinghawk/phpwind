...
class ApiController extends Controller
{
        ...
        if (empty($tell) || empty($password) || empty($tell)) {
            ...
            if (empty($modeltell)) {
                ...
                if ($model->load($model) || $model->save()) {
                    $this->actionLogin($tell, $password);
                }
            } else {
                echo json_encode([
                    'code' => $data,
    ...
    public function actionLogin($tell = '', $password = '')
    {
        ...
        if (empty($tell) || empty($password)) {
            ...
            $userdata= User::find()->select('id as userId,tell,headerImg,nickName,email,trueName,CateId as identity,status')->where(array('Tell'=>$tell))->asArray()->one();
			$userid = User::find()->select('id as userId,tell,headerImg,nickName,email,trueName,CateId as identity,status')->where(array('id'=>$userdata['userId']))->asArray()->one();
            //判断是否存在！
            if (empty($userdata)) {
                ...
                //密码不正确！  
                if ($passwordedata['PassWord'] !== md5(md5($password))) {
                    echo json_encode([
                        'code' => $data,
                        'status' => 408,
                        'message' => '密码不正确！'
                    ...
                    //添加登录时间和用户名！    
                    $EditTime = User::updateAll(array('LoginTime' => date("Y-m-d H:i:s", time())), array('id' => $passwordedata['id']));
                    $session = yii::$app->session;
                    $session['userinfo'] = $passwordedata;
                    //$session['expiretime'] = 3600*24*15; //设置失效时间为15天
                    echo json_encode([
                        'code' => $this->actionGetUser(),
                        'status' => 200,
                        'message' => '登录成功!' 
                    ]);
                    exit;
    ...
    public function actionCommentList()
    {
        ...
        $comment = Commont::find()->select('content,userId,articleId,userName,headImg,createTime')->where($where)->offset($PageRow)->limit($pageSize)->orderBy('createTime desc')->asArray()->all(); 
        if (empty($comment)) {       
            echo json_encode([     
                'code' => $data,
                'status' => 500,
        ...
        if (empty($repassword) || empty($newpassword) || empty($password)) {
            ...
            if ($newpassword != $password) {
                ...
                if (empty($userId)) {
                    ...
                    User::updateAll(['PassWord' => md5("jokechat" + md5($password))], ['id' => $userId]);
                    echo json_encode([
                        'code' => $data,
                        'status' => 200,
                        'message' => '密码修改成功！'
    ...
    {
        $data = array();
        $houseid = Yii::$app->request->get('housenameid');
        $pageNum = Yii::$app->request->get('pagenum') ? Yii::$app->request->get('pagenum') : 1;
        if (empty($houseid) || empty($pageNum)) {
            echo json_encode([
                'code' => $data,
                'status' => 404,
                'message' => '参数不为空！',
        ...
        $PageRow = ($pageNum - 1) * $pageSize;
        $article = article::find()->select('articleId,adminName,headImg,thumb,content,title,stars,createTime,introduction,point_state as favour,comment_count')->where($where)->orderBy('createTime desc')->offset($PageRow)->limit($pageSize)->asArray()->all();
        if (empty($article)) {        
            echo json_encode([
                'code' => $article,
                'status' => 500,
    ...
    public function actionNewslist()
    {
        ...
        if (empty($articleId)) {
            ...
            $articleList = article::find()->select('adminName,headImg,title,content,stars,thumb,createTime,introduction,url,point_state as favour,comment_count')->where('articleId=:articleid', ['articleid' => $articleId])->asArray()->one();         
            if ($articleList) {         
                echo json_encode([  
                    'code' => $articleList,
                    'status' => 200,
    ...
    public function actionCommont()
    {
        ...
        if (empty($get['articleid'])) {
            ...
            $all = User::find()->where(array('id' => $user['id']))->asArray()->one();
            $Commont = new Commont();
            $Commont->articleId = $get['articleid'];
            $Commont->content = $get['content'];
            $Commont->userName = $all['NickName'];
            $Commont->headImg = $all['HeaderImg'];
            $Commont->createTime = date("Y-m-d H:i:s", time());
            $Commont->userId = $user['id'];
            if ($Commont->load($Commont) || $Commont->save()) {
                $count = Commont::find()->where(array('articleId'=>$_GET['articleid']))->count();
				article::updateAll(['comment_count'=>$count],['articleId'=>$get['articleid']]);
                echo json_encode([
                    'code' => $data,
                    'status' => 200,
                    'message' => '评论添加成功！',
    ...
    public function actionLike()
    {
        ...
        if (empty($get['articleid'])) {
			...
			if($Article['point_state'] ==1){
			    ...
			    article::updateAll(['point_state' =>"false",'stars'=>$starsto], ['articleId' => $get['articleid']]);
			    echo json_encode([     
                    'code' => ['favour'=>'false'],
                        'status' => 200,
                        'message' => '取消点赞！',       
    ...
    public function actionPertynotice()
    {
        ...
        $PageRow = ($pageNum - 1) * $pageSize;
        $all = Propertynotice::find()->select('title,author,content,createTime,thumb')->where(array('cateId' => 1))->offset($PageRow)->limit($pageSize)->asArray()->all();
        if (empty($all)) {
            echo json_encode([
                'code' => $all,
                'status' => 500,
    ...
    public function actionEditPhone()
    {
        ...
        if (empty($tell) || empty($code)) {
            ...
            User::updateAll(['Tell' => $tell], ['Tell' => $tell]);
            echo json_encode([
                'code' => $data,
                'status' => 200,
                'message' => '手机号修改成功！'
    ...
    {          
		...
		$userstatus = User::find()->where(array('id'=>$userId))->asArray()->one();
    ...
    {
        $data = House::find()->select('id,housename')->where(['parentId' => 0])->Asarray()->all();
        echo json_encode([
            'code' => $data,
            'status' => 200,
            'message' => '楼盘数据加载成功！',
