...
<div>
	<h1><?=$this->title ?></h1>	
	<form class="form-inline" action="/admin/fushi/get-visitor-record" onsubmit="return checkData();">
		<?php 
			...
			$visitor_phone = empty(Yii::$app->request->get()['visitor_phone']) ? '' : Yii::$app->request->get()['visitor_phone'];
		...
		<div class="form-group">
			<input type="text" class="form-control" name="visitor_phone" id="visitor_phone" value="<?=$visitor_phone ?>" placeholder="请输入访客手机号">
		</div>
		<div class="form-group">
...
<script type="text/javascript">	
	function checkData(){
		...
		if (visitor_phone == undefined || visitor_phone == '') {
	...
	function jumpPage(page,total_page){
		...
		if (checkData()) {
			location.href = '/admin/fushi/get-visitor-record?house_id='+$('#house_id').val()+'&visitor_phone='+$('#visitor_phone').val()+'&stime='+$('#stime').val()+'&etime='+$('#etime').val()+'&page='+page;
		}
	}
</script>
